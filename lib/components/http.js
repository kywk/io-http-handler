...
Http.prototype.start = function (cb) {
  ...
  if (this.useSSL) {
    this.server = https.createServer(this.sslOpts, this.http).listen(this.port, function () {
      self.logger.info('Http handler runs as ' + self.app.get('env') + ' mode. Listening port ' + self.port + ', use HTTPS connection.');
      process.nextTick(cb);
    });
  } else {
    this.server = http.createServer(this.http).listen(this.port, function () {
      self.logger.info('Http handler runs as ' + self.app.get('env') + ' mode. Listening port ' + self.port + ', use HTTP connection.');
      process.nextTick(cb);
    });
  }
};
